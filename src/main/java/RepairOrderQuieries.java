public interface RepairOrderQuieries {
    ...
    String INSERT_QUERY = "insert into `repair_orders` (addDate, isDone, endDate, orderContent, carId)" +
            "values (?, ?, ?, ?, ?)";

    String REPEIR_ORDER_IS_DONE_QUERY = "update `repair_orders` set `isDone` = 1, `endDate`=NOW() where (id = ? and carId = ?)";
